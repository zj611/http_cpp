...
bool start_with(const std::string &a, const std::string &b) {
...
template <typename T>
T get_header_value(const Headers & /*headers*/, const char * /*key*/,
                          size_t /*id*/ = 0, uint64_t /*def*/ = 0) {}

template <>
uint64_t get_header_value<uint64_t>(const Headers &headers,
...
template <typename T>
ssize_t write_content_without_length(Stream &strm,
...
template <typename T>
bool redirect(T &cli, const Request &req, Response &res,
...
template <typename T>
T Response::get_header_value(const char *key, size_t id) const {
  return detail::get_header_value<T>(headers, key, id, 0);
}
...
void Server::set_tcp_nodelay(bool on) { tcp_nodelay_ = on; }
...
void Server::set_keep_alive_timeout(time_t sec) {
...
void Server::set_write_timeout(time_t sec, time_t usec) {
  write_timeout_sec_ = sec;
  write_timeout_usec_ = usec;
}

void Server::set_idle_interval(time_t sec, time_t usec) {
...
ClientImpl::~ClientImpl() { stop_core(); }

bool ClientImpl::is_valid() const { return true; }

Error ClientImpl::get_last_error() const { return error_; }
...
socket_t ClientImpl::create_client_socket() const {
...
bool ClientImpl::send(const Request &req, Response &res) {
...
bool ClientImpl::handle_request(Stream &strm, const Request &req,
...
bool ClientImpl::redirect(const Request &req, Response &res) {
...
bool ClientImpl::write_request(Stream &strm, const Request &req,
...
bool ClientImpl::process_request(Stream &strm, const Request &req,
                                        Response &res, bool close_connection) {
  // Send request
...
bool SSLClient::create_and_connect_socket(Socket &socket) {
...
bool SSLClient::initialize_ssl(Socket &socket) {
...
void SSLClient::close_socket(Socket &socket, bool process_socket_ret) {
  detail::close_socket(socket.sock);
  socket_.sock = INVALID_SOCKET;
  if (socket.ssl) {
